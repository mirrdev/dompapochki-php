jQuery(document).ready(function ($) {
  ...
  function initTable(param = {}) {
    ...
    state.isPromoCodeEnabled =
      !!Number(state.paramCart.promoPersent) &&
      !!state.paramCart.promoValue &&
      !!Number(state.paramCart.promoSum)
        ? state.isPromoCodeEnabled
        : false;
    state.immutableCategory = !excludeCategoryId
      ? state.immutableCategory
      : excludeCategoryId;
    state.activePromocode = !!(
      deliveryType === null || deliveryType === "not delivery"
    );
    state.activePromocode = !state.isPromoCodeEnabled
      ? false
      : state.activePromocode;
    ...
    if (
      ...
      try {
        ...
        if (products.length > 0) {
          ...
          const totalDiscont = products
            .map(({ discont, count }) => discont * count)
            .reduce((acc, el) => acc + el, 0);
          initSum = products
          ...
          if (state.isPromoCodeEnabled) {
            if (deliveryType === "delivery") {
              ...
              if (
                initSum < state.paramCart.promoSum &&
          ...
          if (sum !== null) {
            ...
            if (state.activePromocode) {
              cartTable
                .find(".checkout")
                .before(
                  `<h4 class="text-secondary">Скидка: ${Number(
                    totalDiscont
                  ).toFixed(2)} руб </h4>`
                );
            }
          ...
          if (
            (localStorage.getItem("deliverytype") == undefined &&
  ...
  function addToCartTable(trigger) {
    var productHtml =
      ...
      '" alt = "' +
      trigger.title +
      '" class = "rounded"></a></td><td><strong>' +
      trigger.title +
      "</strong><p>" +
      trigger.detail +
      '</p></td><td class="text-center"><div class="d-inline-block"><div class="btn-group" role="group"><button type="button" class="btn btn-outline-success btn-minus">-</button><div class="form-control-count"><span>' +
      trigger.count +
      '</span><input type="hidden" name="products[prod_' +
      trigger.id +
      "][type_" +
      trigger.type +
      ']" value="' +
      trigger.count +
      '"></div><button type="button" class="btn btn-outline-success btn-plus">+</button></div></div></td><td class="price">' +
      trigger.price +
      " руб</td></tr>";
  ...
  function addProduct(trigger) {
    ...
    var productHtml =
      ...
      '</h3><span class="price">' +
      trigger.price +
      ' руб</span><div class="clearfix"></div><div class="actions"><div>' +
      trigger.detail +
      '</div><a href="#0" class="delete-item">удалить</a></div></div></li>';
    var productAdded = $(productHtml);
  ...
  function addProductInCartTable(product) {
    ...
    var productTotPrice = Number(
      product.find(".price").text().replace("руб", "")
    );
...
function getDiscont(...arr) {
  ...
  if (
    state.isPromoCodeEnabled &&
    price > 0 &&
    state.paramCart.promoPersent > 0
  ) {
    result =
      Number(id) === Number(state.immutableCategory)
        ? 0
...
function isPromocodeValid(state) {
  ...
  return (
    String(state.currentValue).toLocaleLowerCase().replace(/\s+/g, "") ===
    String(state.paramCart.promoValue).toLocaleLowerCase().replace(/\s+/g, "")
  );
}
